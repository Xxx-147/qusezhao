...
def main() -> None:
    ...
    _make_pair_sheet(train_records, manifest_path.parent, run_dir / "01_train_set.jpg", "Training set: negative input -> target positive")
    _make_pair_sheet(test_records, manifest_path.parent, run_dir / "02_test_set.jpg", "Test set: negative input -> target positive")
    metrics = {
    ...
    config = {
        ...
        "train_count": len(train_records),
        "test_count": len(test_records),
        "note": "Each sample is paired: input negative + target positive. Test samples are not used for training.",
    }
...
def _make_pair_sheet(records: list[dict[str, str]], manifest_root: Path, output: Path, title: str) -> None:
    ...
    for row, record in enumerate(records):
        ...
        labels = [f"{row + 1} negative | {record.get('scanner_profile', 'unknown')}", f"{row + 1} target | {record.get('source', 'unknown')}"]
...
def _make_output_sheet(
    ...
    draw = ImageDraw.Draw(sheet)
    draw.text((10, 10), "test: negative / target / manual / adjusted / smart / model", fill=(235, 238, 242))
    ...
    for row, record in enumerate(records):
        ...
        images = [
            ...
            ("model", Image.open(output_paths["model"])),
        ]
    ...
    path.write_text(
        "\n".join(
            [
                ...
                "- BlueNeg raw DNG subset from Hugging Face: ttgroup/blueneg-release.",
                "",
                "Scanner notes:",
                "",
